...
def read_pdf(path_to_pdf):
    ...
    pattern = r"(?i)notification\s*no\.?\s*[:\-]?\s*([A-Z0-9\/\-]+)"
    exam_year = ""
    if all_tables:
        for i in range(0, 2):
            l = all_tables[0][0][i]
...
def Index_file_writer():
    folders = Path("./Results")
...
def read_files_path(This_folder):
    ...
    for path in Input_path:
        Files_in_path = path.rglob("*pdf")
        # break
        if Files_in_path:
            Outpath = Path("./Results") / path.parts[1]
            ...
            for file_path in Files_in_path:
                print(file_path)
                table, not_no = read_pdf(file_path)
                print(not_no.split(":")[-1])
                print(not_no.split(":")[-1].replace("/", "_"))
                print(type(table))
                Output_filename = (
                    Outpath
                    / f"{file_path.parts[1] + '_' + not_no.split(':')[-1].replace('/', '_') + '_'+str(file_num)}.csv"
                )
